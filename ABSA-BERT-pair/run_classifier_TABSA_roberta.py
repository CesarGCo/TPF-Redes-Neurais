...
"""BERT/RoBERTa finetuning runner — modified to support RoBERTa backbone."""
...
def convert_examples_to_features_roberta(examples, label_list, max_seq_length, tokenizer):
    """Converte exemplos para features usando o tokenizer do RoBERTa (BPE).
    
    RoBERTa usa o formato: <s> sentenca_a </s></s> sentenca_b </s>
    em vez do formato BERT: [CLS] sentenca_a [SEP] sentenca_b [SEP]
    
    Nota: RoBERTa nao usa segment_ids (token_type_ids) — sao todos zeros.
    """
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        # RoBERTa nao usa token_type_ids; preenchemos com zeros para manter
...
def main():
    ...
    parser.add_argument("--roberta_model",
    ...
    parser.add_argument("--warmup_proportion",
                        default=0.1,
                        type=float,
                        help="Proporção do treino para warmup linear do learning rate.")
    parser.add_argument("--weight_decay",
                        default=0.01,
                        type=float,
                        help="Weight decay aplicado a todos os parametros exceto bias e LayerNorm.")
    ...
    # Tokenizer RoBERTa (BPE) carregado do diretório local ou do HuggingFace Hub
    ...
    # Modelo RoBERTa para classificação de sequências
    # from_pretrained carrega automaticamente os pesos pré-treinados e adiciona
    # uma camada de classificação linear (num_labels saídas) no topo
    ...
    # Otimizador: AdamW com weight decay explícito nos pesos (exceto bias e LayerNorm)
    # Warmup linear nas primeiras warmup_proportion * num_train_steps iterações
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            # RoBERTa: token_type_ids nao sao usados; passamos apenas input_ids,
