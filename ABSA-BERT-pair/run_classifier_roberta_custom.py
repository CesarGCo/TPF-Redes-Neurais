...
"""RoBERTa finetuning runner com cabeça de classificação customizada.

Modificações em relação ao TPF-01 (BERT original):
1. Backbone trocado de BERT-base para RoBERTa-base (pré-treino mais robusto).
2. Cabeça de classificação customizada: em vez de uma única camada linear,
   usa-se uma MLP com camada oculta intermediária (768 -> 256 -> num_labels),
   ativação ReLU e dropout (0.3), para dar mais capacidade de modelagem e
   regularizar contra o overfitting observado no TPF-01.
"""
...
class RobertaCustomClassifier(nn.Module):
    """RoBERTa com cabeça de classificação customizada (MLP com camada oculta).

    Arquitetura da cabeça:
        [CLS]/<s> hidden (768) -> Dropout -> Linear(768, 256) -> ReLU
        -> Dropout -> Linear(256, num_labels)
    """

    def __init__(self, roberta_model, num_labels, hidden_dim=256, dropout_prob=0.3):
        super(RobertaCustomClassifier, self).__init__()
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        # Usa o hidden state do primeiro token (<s>, equivalente ao [CLS])
        sequence_output = outputs.last_hidden_state  # (batch, seq_len, hidden)
        cls_hidden = sequence_output[:, 0, :]        # (batch, hidden)

        x = self.dropout1(cls_hidden)
        x = self.dense(x)
        x = self.activation(x)
        x = self.dropout2(x)
...
def convert_examples_to_features_roberta(examples, label_list, max_seq_length, tokenizer):
    """Converte exemplos para features usando o tokenizer BPE do RoBERTa."""
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        segment_ids = [0] * max_seq_length  # RoBERTa não usa token_type_ids
...
def main():
    ...
    parser.add_argument("--roberta_model", default="roberta-base", type=str,
                        help="Path to RoBERTa model directory or HuggingFace model name.")
    ...
    parser.add_argument("--hidden_dim", default=256, type=int,
                        help="Tamanho da camada oculta intermediária da cabeça de classificação.")
    parser.add_argument("--dropout_prob", default=0.3, type=float,
                        help="Taxa de dropout na cabeça de classificação.")
    ...
    train_features = convert_examples_to_features_roberta(
    ...
    if args.eval_test:
        ...
        test_features = convert_examples_to_features_roberta(
    ...
    # Modelo com cabeça customizada
    model = RobertaCustomClassifier(
        ...
        if args.eval_test:
            ...
            with open(os.path.join(args.output_dir, f"test_ep_{epoch}.txt"), "w") as f_test:
                for input_ids, input_mask, segment_ids, label_ids in test_dataloader:
                    ...
                    with torch.no_grad():
                        tmp_test_loss, logits = model(input_ids=input_ids,
